...
class ApproxArrayShapingExpr final
    : public Expr,
      private llvm::TrailingObjects<ApproxArrayShapingExpr, Expr *, SourceRange> {
  ...
  ApproxArrayShapingExpr(QualType ExprTy, Expr *Op, SourceLocation L,
                      SourceLocation R, ArrayRef<Expr *> Dims);

  /// Construct an empty expression.
  explicit ApproxArrayShapingExpr(EmptyShell Shell, unsigned NumDims)
      : Expr(ApproxArrayShapingExprClass, Shell), NumDims(NumDims) {}
...
public:
  static ApproxArrayShapingExpr *Create(const ASTContext &Context, QualType T,
                                     Expr *Op, SourceLocation L,
                                     SourceLocation R, ArrayRef<Expr *> Dims,
                                     ArrayRef<SourceRange> BracketRanges);

  static ApproxArrayShapingExpr *CreateEmpty(const ASTContext &Context,
                                          unsigned NumDims);
  ...
  static bool classof(const Stmt *T) {
    return T->getStmtClass() == ApproxArrayShapingExprClass;
  }
...
/// Helper expressions and declaration for ApproxIteratorExpr class for each
/// iteration space.
struct ApproxIteratorHelperData {
...
class ApproxIteratorExpr final
    : public Expr,
      private llvm::TrailingObjects<ApproxIteratorExpr, Decl *, Expr *,
                                    SourceLocation, ApproxIteratorHelperData> {
public:
...
private:
  ...
  ApproxIteratorExpr(QualType ExprTy, SourceLocation IteratorKwLoc,
                  SourceLocation L, SourceLocation R,
                  ArrayRef<IteratorDefinition> Data,
                  ArrayRef<ApproxIteratorHelperData> Helpers);

  /// Construct an empty expression.
  explicit ApproxIteratorExpr(EmptyShell Shell, unsigned NumIterators)
      : Expr(ApproxIteratorExprClass, Shell), NumIterators(NumIterators) {}
  ...
  void setHelper(unsigned I, const ApproxIteratorHelperData &D);
...
public:
  static ApproxIteratorExpr *Create(const ASTContext &Context, QualType T,
                                 SourceLocation IteratorKwLoc, SourceLocation L,
                                 SourceLocation R,
                                 ArrayRef<IteratorDefinition> Data,
                                 ArrayRef<ApproxIteratorHelperData> Helpers);

  static ApproxIteratorExpr *CreateEmpty(const ASTContext &Context,
  ...
  const Decl *getIteratorDecl(unsigned I) const {
    return const_cast<ApproxIteratorExpr *>(this)->getIteratorDecl(I);
  }

  /// Gets the iterator range for the given iterator.
  IteratorRange getIteratorRange(unsigned I);
  const IteratorRange getIteratorRange(unsigned I) const {
    return const_cast<ApproxIteratorExpr *>(this)->getIteratorRange(I);
  }
  ...
  ApproxIteratorHelperData &getHelper(unsigned I);
  const ApproxIteratorHelperData &getHelper(unsigned I) const;

  static bool classof(const Stmt *T) {
    return T->getStmtClass() == ApproxIteratorExprClass;
  }
