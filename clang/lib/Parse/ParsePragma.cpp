...
void PragmaApproxHandler::HandlePragma(Preprocessor &PP,
        PragmaIntroducer Introducer, 
        Token &FirstTok){
    std::cout<<"Starting Processing Approx Pragma" << std::endl;
    ...
    while (Tok.isNot(tok::eod) && Tok.isNot(tok::eof)) {
        ...
        PP.Lex(Tok);
        if (Tok.is(tok::annot_pragma_approx)) {
            PP.Diag(Tok, diag::error_pragma_approx) << "Cannot Handle this token";
        }
        PP.Lex(Tok);
    }
    ...
    std::cout<<"Finished Processing Approx Pragma" << std::endl;
}
